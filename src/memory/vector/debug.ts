 ...
 *
 * 只为「Cài đặt → 向量记忆 → 上次召回详情」面板服务:把上一次召回各阶段的中间结果
...
export interface RecallDebugEmbedHit {
  ...
  queryIndex: number;
  /** 来源标记:Trò chuyện này命中为楼层号「#5」,bundle 快照为「Hồ sơ cũ」 */
  source: string;
  storyTime: string;
  preview: string;
}
...
export interface RecallDebugRerankHit {
  ...
  rerankScore: number;
  similarity: number;
  /** 分档:full=发全文,brief=发Tóm tắt,drop=两档都不达标被丢弃 */
...
export interface RecallDebug {
  ...
  at: number;
  /** 状态文案:'Triệu hồi hoàn tất' | 'Đang tiến hành...' | '未召回:…' | '失败:…' */
  status: string;
  /** 查询重写的Bối cảnh意图(兼作 rerank query) */
  intent: string;
  /** 查询重写产出的多mục检索 query */
  queries: string[];
...
/** Tóm tắt预览:取Tóm tắt文本截断,供面板单行展示(去换行,限长)。 */
...
export function resetRecallDebug(): void {
  Object.assign(recallDebug, empty(), { at: Date.now(), status: 'Đang tiến hành...' });
}

/** 仅更新状态文案(各早退路径写明原因,如「未召回:Viết lại truy vấn未配置」)。 */
