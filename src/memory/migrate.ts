 ...
 * 从旧版 Horae 迁移记忆数据。
 *
 * 关键事实:Horae 与Bách Bảo Thư共用**同一批聊天文件的同一批消息对象**——Horae 把数据挂在
 * `chat[i].horae_meta`,Bách Bảo Thư挂在 `chat[i].extra.bbs_leaf`,两者并存于同一mục msg 上。
 * 所以迁移直接读「当前打开聊天」的 `ctx.chat[i].horae_meta` 产出Bách Bảo Thư数据,不跨文件。
 * 范式照搬 carryover.ts:读源 → 造叶子(挂 extra)+ 森林(写 metadata)+ 把最终状态编码成种子 delta。
 *
 * 模型映射(详见 memory 里的 horae-migration-feature):
 *  - 每个有叙事的 AI 楼 → 一片叶子;text = 该楼非Tóm tắt events 的 summary 拼接。
 *  - Horae 时间是**时间点**(每层一个),Bách Bảo Thư是**时间段**:叶子 timeEnd = 本层时间点,
 *    timeStart = 上一已知时间点(段的左界);缺失则沿用上一已知值(carry-forward)。
 *  - Horae autoSummaries(带 range/depth、嵌套 mergedSummaries)→ Bách Bảo Thư森林 comp 节点:
 *    递归 flatten,depth→level,childIds 由「直接子mục目 + 范围内未被子mục目覆盖的叶子」重建。
 *  - Vật phẩm/计划是 Horae 的**逐层快照**(非增量),按楼序折叠出最终态,整包作为 add-delta 挂到
 *    最后一片叶子(避免逐层重放重复累加)。
 *  - 弃掉Bách Bảo Thư没有的:affection / npcs / costumes / mood / relationships / RPG / customTables。
 */
...
/** 取一层的叙事文本:非Tóm tắt卡 events 的 summary 拼接(Tóm tắt卡 level='Tóm tắt'/isSummary 跳过)。 */
...
const COUNTING_CLASSIFIERS = 'cái chiếc lượng con bức thanh viên bộ quyển cuốn tấm vỉ';
const RE_QTY_PAREN = /[(（]\s*(\d+(?:\.\d+)?)\s*[a-zA-Z一-鿿]*\s*[)）]\s*$/; // (3) (3个) (500g)
const RE_PURE_CLASSIFIER = new RegExp(`[(（][${COUNTING_CLASSIFIERS}][)）]\\s*$`);
// 消耗/归零标记:出现即视为该Vật phẩm已没
const RE_CONSUMED = /[(（](已消耗|已用完|已销毁|已銷毀|消耗殆尽|消耗殆盡|消耗|用尽|用盡|consumed|used\s*up|destroyed|depleted|đã\s*tiêu\s*hao|đã\s*dùng\s*hết|đã\s*phá\s*hủy|đã\s*tiêu\s*hết|tiêu\s*hao|dùng\s*hết|cạn\s*kiệt)[)）]/i;
const RE_ZERO = /[(（]\s*0\s*[a-zA-Z一-鿿]*\s*[)）]\s*$/;

/** 从 Horae Vật phẩm名解析出基础名 + 数量。数量编码在名字括号里(如「苹果(3)」)。 */
...
export interface MigrationPlan {
  ...
  /** 未了结计划/Huyền niệm数 */
  planCount: number;
  /** 当前Bách Bảo Thư是否已有森林数据(将被覆盖) */
  willOverwrite: boolean;
}

/** 把 chat 折叠成最终库存(逐层快照前向合并 + 处理Xóa/消耗)。 */
...
/** 把 chat 折叠成最终「未了结计划/Huyền niệm」。 */
function foldOpenPlans(chat: STMessage[]): StoredDelta['plans'] {
  const deleted = new Set<string>();
  const norm = (s: unknown) => String(s ?? '').trim();
  // 先收集所有Xóa标记(各层 deletedAgenda + chat[0]._deletedAgendaTexts)
  for (const m of chat) {
  ...
  for (const [text, a] of byText) {
    ...
    const kind = a.type === 'Huyền niệm' || a.type === 'Huyền niệm' ? 'suspense' : 'plan';
    add.push({ kind, content: text, createdTime: norm(a.date) || undefined });
  }
  return add.length ? { add } : undefined;
}

/** 递归 flatten Horae autoSummaries(嵌套 mergedSummaries),返回 [mục目, 父id]。 */
...
/** chat 上是否已有Bách Bảo Thư叶子(用于「将覆盖」提示)。 */
...
/**
 * 把当前聊天里的 Horae 数据迁移成Bách Bảo Thư数据。返回是否成功。
...
export async function runHoraeMigration(): Promise<boolean> {
  ...
  if (!ctx.getCurrentChatId?.()) {
    toast('Vui lòng mở một cuộc trò chuyện trước khi di chuyển', 'warning');
    return false;
  }
  const chat = ctx.chat ?? [];
  if (!chat.length) {
    toast('Cuộc trò chuyện hiện tại đang trống, không có dữ liệu để di chuyển', 'warning');
    return false;
  }

  const plan = computeMigrationPlan();
  if (!plan.hasData) {
    toast('Không phát hiện dữ liệu cũ Horae trong cuộc trò chuyện hiện tại', 'warning');
    return false;
  }
  ...
  try {
    ...
    // ===== 2. 把最终状态(Vật phẩm/计划/时间/地点)编码成种子 delta,挂到最后一片叶子 =====
    // 若没有任何叙事叶子但有状态,造一mục空文本叶子挂到最后一个 AI 楼承载它。
    const items = foldFinalItems(chat);
    const plans = foldOpenPlans(chat);
    ...
    // 子mục目集合:供「直接覆盖范围内、未被子mục目覆盖的叶子」计算 childIds。
    ...
    for (const { entry } of flat) {
      ...
      // childIds:① 直接子mục目 id(下层 comp);② 本范围内、未被任何子mục目范围覆盖的叶子 id。
      ...
      if (range) {
        for (let i = range[0]; i <= range[1]; i++) {
          ...
          if (!leafId) continue;
          if (childRanges.some(([a, b]) => i >= a && i <= b)) continue; // 已属某子mục目
    ...
    // 检测一次隐藏:把已被Tóm tắt覆盖、滚出保留窗口的旧楼层隐藏掉(复用Tóm tắt收尾同款逻辑),
    // 避免迁移后全文与Tóm tắt在上下文里重复;内部已含刷新注入。
    await syncHiddenNow();

    toast(
      `Di chuyển hoàn tất: ${leafIdByFloor.size} lá / ${newSummaries.length} tóm tắt / ${items.length} vật phẩm / ${plans?.add?.length ?? 0} kế hoạch`,
      'success',
    );
    return true;
  } catch (e) {
    toast(`Di chuyển thất bại: ${e instanceof Error ? e.message : String(e)}`, 'error');
    console.error('[Bách Bảo Thư] Di chuyển từ Horae thất bại:', e);
    return false;
  }
}

/** 规整 depth → Bách Bảo Thư level(≥1)。 */
