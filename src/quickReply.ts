...
const BAR_CLASS = 'bbs-qr-bar'; // 仅当 #qr--bar 不存在时,我们自建的mục加这个标记类,便于回收

/**
 * 往聊天框上方注入一个「Bách Bảo Thư」按钮,外观与 ST 原生快速回复按钮**完全一致**。
 *
 ...
 *
 * 挂载结构对齐原生:#qr--bar(已存在则复用,否则自建一个 .flex-container.flexGap5 mục)
 *   └ .qr--buttons(容器)
 *       └ .qr--button(我们的按钮)
 *
 * 受Cài đặt开关控制:开 → 确保已注入;关 → 移除。轮询等聊天框出现(SPA 懒加载)。
 *
 * 注意:.qr--button 的样式全靠祖先选择器 `#qr--bar > .qr--buttons .qr--button` 生效,
 * 按钮**必须**落在该结构内才有原生外观。而 ST 的快速回复会在切聊天/改Cài đặt时重建 #qr--bar,
 * 把我们的按钮一并冲掉——故用 MutationObserver 盯住 #send_form,被冲掉就补回(同 JS-Slash-Runner)。
 */
let pollTimer: ReturnType<typeof setInterval> | null = null;
let observer: MutationObserver | null = null;

/** 取(或建)#qr--bar:优先复用 ST 原生那mục,没有才自建一mục同款。返回挂按钮用的 .qr--buttons 容器。 */
function ensureButtonHolder(sendForm: HTMLElement): HTMLElement {
  ...
  // 复用mục内已有的 .qr--buttons,没有就建一个(原生未组合模式时可能没有)
...
function buildButton(): HTMLElement {
  ...
  btn.title = 'Mở Bách Bảo Thư';
  btn.textContent = 'Bách Bảo Thư';
...
function removeButton(): void {
  ...
  // 若是我们自建的空 #qr--bar(原生没有 QR),按钮撤了就一并回收,别留空mục
